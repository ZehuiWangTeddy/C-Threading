...
public class TaskUpdateService : ITaskUpdateNotifier, IDisposable
{
    ...
    private readonly Dictionary<int, List<EventHandler<TaskUpdateEventArgs>>> _taskSubscribers = new();
    ...
    public void NotifyExecutionTimeChanged(BaseTask task)
    {
        ...
        TaskExecutionTimeChanged?.Invoke(this,
            new TaskUpdateEventArgs(task, TaskUpdateType.ExecutionTimeChanged, "ExecutionTime"));
    ...
    public void NotifyTaskDeleted(BaseTask task)
    {
        ...
        if (_taskSubscribers.ContainsKey(task.Id)) _taskSubscribers.Remove(task.Id);
    }
    ...
    public void UnsubscribeFromTask(int taskId, EventHandler<TaskUpdateEventArgs> handler)
    {
        ...
        if (_taskSubscribers.ContainsKey(taskId))
        {
            ...
            if (_taskSubscribers[taskId].Count == 0) _taskSubscribers.Remove(taskId);
        }
    }
