...
public class AddTaskViewModel : ObservableObject
{
    ...
    private async void ConfirmAsync()
    {
        if (ValidateInput())
        {
            ...
            WeakReferenceMessenger.Default.Send(new NavigationMessage(nameof(AddTaskViewModel), nameof(AddTaskPage),
                true));
        }
    }
    ...
    private async Task SaveTaskToDatabase()
    {
        try
        {
            var executionTime = new ExecutionTime(
                SelectRecurrencePattern == RecurrencePattern.OneTime
                    ? ExecutionDate.Date.Add(ExecutionTimeSpan)
                    : null,
                SelectRecurrencePattern,
                NextRunDate.Date.Add(NextRunTimeSpan)
            );
        ...
        {
            WeakReferenceMessenger.Default.Send(new ToastMessage(nameof(AddTaskViewModel), nameof(AddTaskPage),
                $"Save Faild: {ex.Message}"));
        }
    }
    ...
    private bool ValidateInput()
    {
        ...
        {
            WeakReferenceMessenger.Default.Send(new ToastMessage(nameof(AddTaskViewModel), nameof(AddTaskPage),
                "Execution time cannot early than now"));
            return false;
        }
        ...
        if (ShowNextRunTime)
        {
            ...
            {
                WeakReferenceMessenger.Default.Send(new ToastMessage(nameof(AddTaskViewModel), nameof(AddTaskPage),
                    "Next run time must late than execution time"));
                return false;
            }
        }
        ...
        switch (SelectedTaskType)
        {
            ...
            case "File Compression Task":
                ...
                {
                    WeakReferenceMessenger.Default.Send(new ToastMessage(nameof(AddTaskViewModel),
                        nameof(AddTaskPage), "Input File Directory"));
                    return false;
                }
            ...
            case "File Backup System Task":
                ...
                {
                    WeakReferenceMessenger.Default.Send(new ToastMessage(nameof(AddTaskViewModel),
                        nameof(AddTaskPage), "Input SourceDirectory and TargetDirectory"));
                    return false;
                }
            ...
            case "Email Notification Task":
                ...
                {
                    WeakReferenceMessenger.Default.Send(new ToastMessage(nameof(AddTaskViewModel),
                        nameof(AddTaskPage), "Input SenderEmail, ReceiverEmail and EmailSubject"));
                    return false;
                }
    ...
    {
        get => _fileDirectory;
        set => SetProperty(ref _fileDirectory, value);
    }
    ...
    {
        get => _executionDate;
        set => SetProperty(ref _executionDate, value);
        //CalculateNextRunTime();
    }
    ...
    public TimeSpan NextRunTimeSpan
    {
        ...
        private set => SetProperty(ref _nextRunTimeSpan, value);
    }
